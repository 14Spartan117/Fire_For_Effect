...
def _build_lx(qx):
    """Build lx (survivors) table from qx (death probability) table."""
    lx = {0: 1.0}
...
def actuarial_life_expectancy(start_age, sex='male'):
    """Expected remaining years of life from start_age."""
    lx = LX_MALE if sex == 'male' else LX_FEMALE
    lx_start = lx.get(start_age, 1e-9)
    total = 0.0
    for age in range(start_age, 120):
        total += lx.get(age, 0) / lx_start
    return total
