    ...
    with container:
        ...
        with col_l:
            ...
            start_rank = st.selectbox(
                "Current Rank", RANKS,
                index=RANKS.index(ss["tab2_rank_widget"]) if ss["tab2_rank_widget"] in RANKS else 4,
                key="tab2_rank_widget",
            )
            start_tis = st.number_input(
                "Current Years of Service",
                min_value=0.0, max_value=40.0,
                value=ss["tab2_tis_widget"], step=0.5,
                key="tab2_tis_widget",
            )
...
*May the odds be ever in your favor.*
            ...
            if st.button("Run Simulation", type="primary"):
                ...
                with st.spinner("Running 1,000 trials..."):
                    ...
                    if data_source == "ETF Proxy (Real)":
                        st.info(
                            "\u2139\uFE0F TSP.gov blocked direct access. "
                            "Using real ETF proxy returns: "
                            "SPY (C), VXF (S), EFA (I), AGG (F), SHY (G)."
                        )
                    ...
                    sim_results = run_real_monte_carlo(
                        ...
                        explore_contrib_schedule, hist_returns,
...
**Monte Carlo** -- 1,000 trials. Each trial randomly samples historical monthly TSP returns (with replacement). Success = portfolio >= target nest egg at your stop-working age.
